...
{
    /// <summary>
    /// Поле fromCol типа 'char' предназначено для передачи координат по горизонтали(столбцов) двумерного массива в виде литеры
    ///  английского алфавита от 'a' до 'h' и относится к клетке ИЗ которой делается ход.    
    /// </summary>
    char fromCol;
    /// <summary>
    /// Поле toCol типа 'char' предназначено для передачи координат по горизонтали(столбцов) двумерного массива в виде литеры
    /// английского алфавита от 'a' до 'h' и относится к клетке В которую делается ход.
    /// </summary>
    char toCol;
    /// <summary>
    /// Поле fromRow типа 'int' предназначено для передачи координат по вертикали(рядов) двумерного массива в виде целых 
    /// чисел от 1 до 8. Относится к клетке ИЗ которой делается ход.
    /// </summary>
    int fromRow;
    /// <summary>
    /// Поле toRow типа 'int' предназначено для передачи координат по вертикали(рядов) двумерного массива в виде целых 
...
int main() {
    ...
    while (true)
    {
        ...
        cout << "\t\t\t\t\t\t\t\t\t\t";
        cout << "\033[36m";//задаем цвет текста ввода, отличный от остального текста с помощью эскейп-последовательности
        ...
        turn.fromCol = turn.fromCol - 'a';//буква 'а' в аскии имеет код 97, итд поэтому отнимая это число мы получаем индекс
